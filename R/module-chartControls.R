...
chartControlsServer <- function(input, output, session, type, data = NULL) {
  ...
  observeEvent(type$x, {
    ...
    if (type$x %in% c("point", "line", "density", "violin")) {
      toggleDisplayServer(session = session, id = ns("controls-xtrans"), display = "block")
    } else {
      toggleDisplayServer(session = session, id = ns("controls-xtrans"), display = "none")
    }
    if (type$x == "sf") {
      toggleDisplayServer(session = session, id = ns("controls-ytrans"), display = "none")
    } else {
      toggleDisplayServer(session = session, id = ns("controls-ytrans"), display = "block")
    }
  })
